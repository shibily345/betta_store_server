...
class FoodsController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('is_recommended', __('Is Recommended'))->switch();
        $grid->column('name', __('Name'));
        $grid->column('breeder', __('Breeder'));
         $grid->column('FoodType.title', __('Category'));
        $grid->column('price', __('Price'));
        $grid->column('malePrice', __('malePrice'));
        $grid->column('femalePrice', __('femalePrice'));
        //$grid->column('location', __('Location'));
        $grid->column('stars', __('Stars'));
        $grid->column('img', __('Thumbnail Photo'))->image('',60,60);
      //  $grid->column('video', __('Thumbnail Video'))->file('',60,60);
    ...
    protected function form()
    {
        ...
        $form->text('breeder', __('Breeder'));
          $form->select('type_id', __('Type_id'))->options((new FoodType())::selectOptions());
        $form->number('price', __('Price')); 
        $form->number('malePrice', __('malePrice'));
        $form->number('femalePrice', __('femalePrice'));
        
       // $form->text('location', __('Location'));
        $form->switch('is_recommended', __('Is Recommended'))->default(0);
        ...
        //$form->video('video', __('Thumbnail video'))->uniqueName();
